...
vnTRUST.ajax_popup = function (url, method, param, callback, option) {
    ...
    jQuery.ajax({
        ...
        success: function (xhr) {
            ...
            if (xhr && vnTRUST.is_exists(xhr.intReturn)) {
                switch (xhr.intReturn) {
                    case -1: vnTRUST.show_popup_message(xhr.msg, "ThĂ´ng bĂ¡o lá»—i!", -1); break;
                    case 0: vnTRUST.show_popup_message(xhr.msg, "Cáº£nh bĂ¡o", 0); break;
                    case 1: vnTRUST.show_popup_message(xhr.msg, "ThĂ´ng bĂ¡o", 1); break;
                }
            }
        ...
        error: function (xhr) {
            ...
            delete vnTRUST._store.ajax[url];
            opt.error();
            //vnTRUST.show_popup_message("Lá»—i káº¿t ná»‘i máº¡ng", "ThĂ´ng bĂ¡o lá»—i!", -1);
        }
    });
};
...
vnTRUST.show_loading = function (txt) {
    txt = vnTRUST.is_str(txt) ? txt : 'Ä�ang táº£i dá»¯ liá»‡u...';
...
vnTRUST.confirm = function (message, callback, callback_data) {
    ...
    vnTRUST.show_next_popup(
	  ...
	  ('<div align="center"><input type="button" value="Ä�á»“ng Ă½" onclick="vnTRUST.confirm_ok()" />&nbsp;&nbsp;&nbsp;')
	  ('<input type="button" value="Há»§y bá»�" onclick="vnTRUST.hide_popup(\'popup_confirm\')" /></div>')(),
      ...
      function (j) {
          if (j.err == 0) {
              vnTRUST.cart.theme.systemAlert(
                vnTRUST.join
                  ('<div style="font-size:14px;margin-top:5px">XoĂ¡ cache thĂ nh cĂ´ng</div>')()
                  , 'Há»‡ thá»‘ng'
              );
          }
      });
    ...
    ('<div class="classic-popup"' + style + '>')
        ...
        ('<div class="classic-popup-main">')
            ('<div class="classic-popup-title">')
                ...
                ('<a href="javascript:void(0)" class="classic-popup-close" title="Ä�Ă³ng" onclick="vnTRUST.hide_overlay_popup(\'' + id + '\');' + close + '">x</a>')
                ('<div class="c"></div>')
            ('</div>')
